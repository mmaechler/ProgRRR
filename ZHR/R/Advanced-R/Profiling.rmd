...
(I use `pause()` instead of `Sys.sleep()` because `Sys.sleep()` does not appear in profiling outputs because as far as R can tell, it doesn't use up any computing time.) \indexc{pause()}
...
```{r, echo = FALSE}
embed_png("screenshots/profiling-lineprof-f.png", dpi = 220)
```
...
```{r, echo = FALSE}
embed_png("screenshots/profiling-lineprof-g.png", dpi = 220)
```

Then `h()`:

```{r, echo = FALSE}
embed_png("screenshots/profiling-lineprof-h.png", dpi = 220)
```
...
* [_Parallel Computing for Data Science_](http://heather.cs.ucdavis.edu/paralleldatasci.pdf) by
  Norm Matloff.
